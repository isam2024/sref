/* app.js - Main application logic */
    ...
    class App {
      ...
      generateRandomSet(randomnessLevel = 500) {
        ...
        let base = Math.floor(Math.random() * (range - variance + 1)) + min;
          ...
          if (sets.length === 0) {
            ...
            sets.forEach((set) => {
              ...
              deleteBtn.addEventListener('click', async () => {
                if (
                  confirm(
                    `Are you sure you want to delete set ${set.baseNumber}?`
                  )
                ) {
      ...
      async loadRandomImages() {
        ...
        try {
          ...
          while (
            randomImages.length < numImages &&
            usedIndices.size < images.length
          ) {
          ...
          imageGrid.style.gridTemplateColumns = `repeat(${Math.min(
            3,
            randomImages.length
          )}, 1fr)`;
        } catch (error) {
      ...
      sortSets(sortBy) {
        ...
        const clickedButton = document.querySelector(`.sort-button[onclick="app.sortSets('${sortBy}')"]`);
        if (clickedButton) {
          clickedButton.classList.add('active');
        }

        this.currentSortOrder = sortBy; // Update current sort order
        this.loadSavedSetsWithSort(); // Reload sets with the new sort order
      }

      async loadSavedSetsWithSort() {
        try {
          let sets = await this.db.getAllSets();

          // Sort sets based on the selected criteria
          if (this.currentSortOrder === 'date-added-newest') {
            sets.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
          } else if (this.currentSortOrder === 'date-added-oldest') {
          ...
          .addEventListener('click', async () => {
            try {
              ...
              if (success) {
                ...
                this.showMessage(
                  `Generated random set starting at: ${baseNumber}`
                );
                notesInput.value = '';
              }
            } catch (error) {
